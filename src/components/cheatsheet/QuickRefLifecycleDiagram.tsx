    ...
    <div className="relative pl-3">
      ...
      <div className="pb-3">
        <div className="flex items-baseline gap-2">
          <span className="text-xs font-bold" style={{ color: phase.accentHex }}>
            {phase.phase}
          </span>
          <span className="text-[11px] text-slate-400">{phase.desc}</span>
        </div>
        <div className="mt-1 flex flex-col gap-0.5">
          {phase.hooks.map((hook) => (
            <span key={hook} className="font-mono text-[11px] text-slate-300">
              {hook}
            </span>
          ))}
        </div>
        <div className="mt-1 text-[10px] italic text-slate-500">{phase.classic}</div>
      </div>
    </div>
  )
}
