  ...
  "sections": [
    {
      ...
      interviewQuestions: [
        ...
        {
          question: `Як ти обираєш, який AI-інструмент для кодування використати на проекті, і як показати це на співбесіді?`,
          answer: `Аргументуй вибір контекстом: workflow (CLI-first vs IDE-first), потрібним обсягом контексту проекту, та вимогами до приватності — а не просто "цей популярніший". На демонстрації корисно показати конкретику: "Я використовую Claude Code для цього проекту, оскільки CLAUDE.md документує нашу архітектуру" звучить переконливіше за загальні фрази. Формулюй роль інструмента як помічника: не "AI пише мій код", а "AI як pair programmer — рішення приймаю я".`,
        },
      ],
      "blocks": [
        {
    ...
    },
    {
      ...
      interviewQuestions: [
        ...
        {
          question: `Як швидко оцінити, на якому рівні зрілості перебуває скіл, яким ти вже користуєшся?`,
          answer: `Відкрий його й подивись, чи є поруч evals — якщо ні, це рівень 1-2 (і це нормально, там зараз більшість індустрії). П'ять тестових промтів (кілька happy-path і кілька negative-кейсів) можна написати за один вечір без додаткового інструментарію — це вже мінімальний, але реальний крок до наступного рівня зрілості.`,
        },
      ],
      "blocks": [
        {
