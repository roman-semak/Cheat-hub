...
| 5 | JSX та рендеринг: події, списки, умовний рендер, refs, fragments, render props | `/react#fundamentals-components-jsx` (Fragment), `#jsx-synthetic-events`, `#fundamentals-lists-conditionals`, `#hooks-useref`, `#patterns` (render props, HOC) | `src/components/cheatsheet/ContentBlocks.tsx` (рендер за типом блока), `CodeEditor.tsx` (ref на Monaco) | ✅ |
